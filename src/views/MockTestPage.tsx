...
export const MockTestPage: React.FC = () => {
    ...
    const interval = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          showToast(language === 'hi' ? 'समय समाप्त! आपकी परीक्षा स्वतः जमा हो गई है।' : 'Time up! Your test has been automatically submitted.', 'info');
          return 0;
        }
        return prev - 1;
      });
  ...
  const handleSelectOption = (optionIndex: number) => {
    ...
    setUserAnswers({ ...userAnswers, [currentQ.id]: optionIndex });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <Breadcrumb items={[{ label: language === 'hi' ? 'ऑनलाइन अभ्यास परीक्षा (मॉक टेस्ट)' : 'Interactive Mock Test' }]} />
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white dark:bg-[#1A1414] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-md">
        <div>
          <span className="bg-[#C79A2D]/10 text-[#C79A2D] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
            {language === 'hi' ? 'राष्ट्रीय स्तर मॉडल टेस्ट' : 'National Model Test'}
          </span>
          <h1 className="font-playfair text-2xl font-bold text-gray-900 dark:text-white mt-1">
            {subject === 'Hindi' ? (language === 'hi' ? 'हिंदी भाषा अभ्यास परीक्षा' : 'Hindi Language Practice Test') : (language === 'hi' ? 'संस्कृत भाषा अभ्यास परीक्षा' : 'Sanskrit Practice Exam')}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
            <button
              onClick={() => { setSubject('Hindi'); handleResetExam(); }}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                subject === 'Hindi' ? 'bg-[#7B1E1E] text-white' : 'text-gray-600 dark:text-gray-300'
              }`}
            >
              Hindi
            </button>
            <button
              onClick={() => { setSubject('Sanskrit'); handleResetExam(); }}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                subject === 'Sanskrit' ? 'bg-[#C79A2D] text-[#7B1E1E]' : 'text-gray-600 dark:text-gray-300'
              }`}
            >
              Sanskrit
            </button>
          ...
          {/* Timer Clock */}
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-extrabold shadow-sm ${
            timeLeft < 120 ? 'bg-red-500 text-white animate-pulse' : 'bg-[#1A1414] text-[#C79A2D] border border-[#C79A2D]/30'
          }`}>
        ...
        <div className="bg-white dark:bg-[#1A1414] rounded-3xl p-8 sm:p-12 border-2 border-[#C79A2D]/40 shadow-2xl space-y-8 animate-in fade-in duration-300">
          <div className="text-center space-y-3">
            ...
            <h2 className="font-playfair text-3xl font-bold text-gray-900 dark:text-white">
              {language === 'hi' ? 'अभ्यास परीक्षा परिणाम' : 'Mock Exam Performance Result'}
            </h2>
          ...
          <div className="space-y-6 pt-4 border-t border-gray-100 dark:border-gray-800">
            <h3 className="font-bold text-base text-gray-900 dark:text-white">
              {language === 'hi' ? 'उत्तर व्याख्या व विश्लेषण' : 'Detailed Question Analysis'}
            </h3>
                ...
                <div key={q.id} className={`p-5 rounded-2xl border ${
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      isCorrect ? 'bg-[#2E8B57] text-white' : 'bg-red-600 text-white'
                    }`}>
                  ...
                  <div className="mt-3 text-xs space-y-1">
                    <div className="text-gray-600 dark:text-gray-300">
                      <strong>Your Answer:</strong> {selectedOpt !== undefined ? q.options[language][selectedOpt] : 'Not Attempted'}
                    </div>
                    <div className="text-[#2E8B57] font-bold">
                      <strong>Correct Option:</strong> {q.options[language][q.correctIndex]}
                    </div>
          ...
          <div className="text-center pt-4">
            <button 
              ...
              className="bg-[#7B1E1E] text-white font-bold px-8 py-3 rounded-2xl text-xs flex items-center gap-2 mx-auto"
            >
              <RefreshCw className="w-4 h-4" />
              <span>{language === 'hi' ? 'पुनः परीक्षा दें' : 'Retake Exam'}</span>
            </button>
          </div>
        </div>
      ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 bg-white dark:bg-[#1A1414] rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-gray-800 shadow-xl space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl border text-xs font-semibold transition-all flex items-center justify-between ${
                      isSelected 
                        ? 'bg-[#7B1E1E] text-white border-[#C79A2D] shadow-md' 
                        : 'bg-gray-50 dark:bg-gray-800/60 text-gray-800 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:border-[#C79A2D]'
                    }`}
                  >
            ...
            <div className="flex items-center justify-between pt-6 border-t border-gray-100 dark:border-gray-800">
              <button 
                ...
                className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs font-bold disabled:opacity-40"
              >
              ...
              {currentIdx < questions.length - 1 ? (
                <button 
                  ...
                  className="bg-[#7B1E1E] text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-md hover:bg-[#541313]"
                >
                ...
                <button 
                  ...
                  className="bg-[#2E8B57] text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-md hover:bg-emerald-700"
                >
                  {language === 'hi' ? 'परीक्षा जमा करें' : 'Submit Final Exam'}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-4 bg-white dark:bg-[#1A1414] rounded-3xl p-6 border border-gray-100 dark:border-gray-800 shadow-xl space-y-4">
            <h3 className="font-bold text-xs uppercase tracking-wider text-gray-500">
              {language === 'hi' ? 'प्रश्न तालिका (Question Palette)' : 'Question Status Matrix'}
            </h3>
                ...
                return (
                  <button
                    ...
                    className={`h-10 rounded-xl font-bold text-xs flex items-center justify-center transition-all ${
                      isCurrent 
                        ? 'ring-2 ring-[#C79A2D] bg-[#7B1E1E] text-white scale-105' 
                        : isAnswered 
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-800 text-[11px] space-y-2 text-gray-500">
              <div className="flex items-center gap-2">
                ...
                <span>{language === 'hi' ? 'उत्तर दिया (Attempted)' : 'Answered'}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded bg-gray-200 dark:bg-gray-700"></span>
                <span>{language === 'hi' ? 'शेष (Unanswered)' : 'Unanswered'}</span>
              </div>
            </div>

            <button 
              onClick={() => setIsExamSubmitted(true)}
              className="w-full mt-4 bg-gradient-to-r from-[#7B1E1E] to-[#A32A2A] text-white py-3 rounded-xl font-bold text-xs shadow-md"
            >
              {language === 'hi' ? 'उत्तर तालिका जमा करें' : 'Finish & View Score Card'}
            </button>
          </div>
