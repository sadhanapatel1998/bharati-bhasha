...
import SanskritsyllabusAccordion from './SanskritsyllabusAccordion';
...
const SKILLS = [
  { icon: BookOpenCheck, label: 'व्याकरणम्', color: 'from-rose-500 to-pink-500' },
  { icon: MessagesSquare, label: 'पठन-बोधनम्', color: 'from-blue-500 to-cyan-500' },
  { icon: SpellCheck2, label: 'शब्दभण्डारः', color: 'from-emerald-500 to-teal-500' },
  { icon: Brain, label: 'सामान्यज्ञानम्', color: 'from-purple-500 to-fuchsia-500' },
];
...
const AVAILABLE_SAMPLE_GRADES: string[] = [];
...
const GRADES = Array.from({ length: 10 }, (_, i) => {
  ...
  return {
    ...
    samplePaperHref: `/downloads/sanskrit-sample-paper-class-${id}.pdf`,
    ...
    relative">
      ...
      sm:mx-0 sm:px-0 sm:overflow-visible sm:flex-wrap sm:justify-center"
        ...
        <div id="skills" className="scroll-mt-24 space-y-6">
          ...
          <SectionHeading
            icon={Sparkles}
            title="संस्कृत ओलंपियाड में परखे जाने वाले कौशल"
                  ...
                  ${isOpen ? 'rotate-6 scale-110' : 'group-hover:rotate-6 group-hover:scale-110'}`}
                    ...
                    ${isAvailable
                        ...
                        <p className="text-sm text-slate-500 font-medium mt-0.5 flex items-center gap-2">
                          <span>{'भारती भाषा ओलंपियाड · संस्कृत'}</span>
                          {!isAvailable && (
