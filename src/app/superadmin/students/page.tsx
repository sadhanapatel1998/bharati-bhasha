...
export default function SuperAdminStudents() {
  const { t, d } = useI18n();
  const { api } = usePanel();
  const r = useResource<StudentRow>('/api/superadmin/students');
  ...
  useEffect(() => {
    api<{ items: { _id: string; name: string; code: string }[] }>('/api/superadmin/schools?limit=200', { silent: true })
  ...
  const save = async () => {
    if (!form.name || !form.classLevel || !form.schoolId) return;
    const ok = modal.row
  ...
  const statusOptions = [
    { value: 'registered', label: t('student.status.registered') },
    { value: 'rollAllotted', label: t('student.status.rollAllotted') },
    { value: 'hallTicket', label: t('student.status.hallTicket') },
    { value: 'appeared', label: t('student.status.appeared') },
  ];

  return (
    <>
      <PageHeader title={t('student.title')} subtitle={t('student.subtitle')}>
        <Button
          variant="secondary"
          icon={Download}
          onClick={() =>
            downloadCSV('students.csv', r.items as unknown as Record<string, unknown>[], [
      ...
      <Card>
        ...
        {r.loading ? (
          ...
          <EmptyState action={<Button icon={Plus} onClick={openCreate}>{t('student.addTitle')}</Button>} />
        ) : (
          <TableWrap>
            <thead>
              <tr>
            ...
            <tbody>
              {r.items.map((row) => (
                <tr key={row._id} className="transition hover:bg-stone-50/70 dark:hover:bg-white/[0.03]">
                  ...
                  <Td className="max-w-[220px] truncate text-xs">{row.schoolName || '—'}</Td>
                  <Td>
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label={t('student.school')} required>
            <Select
              value={form.schoolId}
              onChange={(e) => set('schoolId', e.target.value)}
              placeholder={t('common.selectPlaceholder')}
              options={schools.map((s) => ({ value: s._id, label: `${s.code} — ${s.name}` }))}
            />
          </Field>
          <Field label={t('student.examCenter')}>
