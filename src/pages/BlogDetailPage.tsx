...
export const BlogDetailPage: React.FC = () => {
  const { language, routeParams, navigateTo, showToast } = useApp();
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <Breadcrumb items={[
        { label: language === 'hi' ? 'ब्लॉग' : 'Blogs', route: 'blogs' },
        { label: post.title[language] }
      ]} />

      <button 
        onClick={() => navigateTo('blogs')}
        className="inline-flex items-center gap-1.5 text-xs font-bold text-[#7B1E1E] dark:text-[#C79A2D] hover:underline"
      >
        <ArrowLeft className="w-4 h-4" />
        <span>{language === 'hi' ? 'सभी ब्लॉग पर वापस जाएँ' : 'Back to Blogs'}</span>
      </button>
      ...
      <div className="prose dark:prose-invert max-w-none text-xs sm:text-sm text-gray-700 dark:text-gray-300 leading-relaxed space-y-4">
        ...
        <p>
          {language === 'hi'
            ...
            : 'Fostering language mastery early in life builds cognitive resilience, analytical precision, and deep-rooted cultural pride in young scholars.'}
        </p>
      </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-2">
          {post.tags.map((tg, i) => (
            <span key={i} className="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-[10px] font-semibold">
              #{tg}
            </span>
          ))}
        </div>
        ...
        <button 
          onClick={() => showToast(language === 'hi' ? 'ब्लॉग लिंक कॉपी कर दिया गया है!' : 'Article link copied to clipboard!', 'info')}
