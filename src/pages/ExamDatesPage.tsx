...
export const ExamDatesPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
      <Breadcrumb items={[{ label: language === 'hi' ? 'परीक्षा तिथियाँ' : 'Exam Dates' }]} />

      <div className="text-center max-w-3xl mx-auto space-y-4">
        <span className="bg-[#C79A2D]/10 text-[#C79A2D] px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider">
          {language === 'hi' ? 'समय सारणी 2026' : 'Schedule 2026'}
        </span>
        <h1 className="font-playfair text-3xl sm:text-5xl font-bold text-gray-900 dark:text-white">
          {language === 'hi' ? 'ओलंपियाड परीक्षा तिथियाँ व समय सारणी' : 'Exam Schedule & Key Dates 2026'}
        </h1>
      </div>

      <div className="space-y-4">
        {EXAM_SCHEDULE.map((item) => (
          <div key={item.id} className="bg-white dark:bg-[#1A1414] rounded-3xl p-6 border border-gray-100 dark:border-gray-800 shadow-md flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <button className="bg-[#7B1E1E] text-white font-bold px-6 py-2.5 rounded-xl text-xs shrink-0">
              {language === 'hi' ? 'कैलेंडर में जोड़ें' : 'Add to Calendar'}
            </button>
          </div>
        ))}
      </div>
