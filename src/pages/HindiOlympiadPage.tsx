...
export const HindiOlympiadPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
      <Breadcrumb items={[{ label: language === 'hi' ? 'हिंदी भाषा ओलंपियाड' : 'National Hindi Olympiad' }]} />
      ...
      <div className="bg-gradient-to-r from-[#7B1E1E] to-[#A32A2A] text-white rounded-3xl p-8 sm:p-12 shadow-2xl relative overflow-hidden">
        <div className="max-w-3xl space-y-4">
          ...
          <h1 className="font-playfair text-3xl sm:text-5xl font-bold">
            {language === 'hi' ? 'राष्ट्रीय हिंदी भाषा ओलंपियाड 2026' : 'National Hindi Bhasha Olympiad 2026'}
          </h1>
          <p className="text-xs sm:text-base text-gray-200 leading-relaxed">
            {language === 'hi' 
              ...
              : 'Empowering students across India with advanced Hindi vocabulary, grammar precision, spelling accuracy, and literary appreciation.'}
          </p>
          <div className="pt-4 flex flex-wrap gap-4">
            ...
            >
              {language === 'hi' ? 'हिंदी ओलंपियाड हेतु पंजीकरण करें' : 'Register for Hindi Olympiad'}
            </button>
            <button 
            ...
            >
              {language === 'hi' ? 'मॉक टेस्ट दें' : 'Take Free Practice Test'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-[#1A1414] p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-2">
          ...
          <h3 className="font-bold text-sm text-gray-900 dark:text-white">{language === 'hi' ? 'प्रथम चरण तिथि' : 'Phase 1 Exam Date'}</h3>
          <p className="text-xs text-gray-500">October 18, 2026 (10:00 AM IST)</p>
        </div>

        <div className="bg-white dark:bg-[#1A1414] p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-2">
          <Clock className="w-6 h-6 text-[#C79A2D]" />
          <h3 className="font-bold text-sm text-gray-900 dark:text-white">{language === 'hi' ? 'समय अवधि व अंक' : 'Duration & Marks'}</h3>
          <p className="text-xs text-gray-500">60 Minutes | 100 Total Marks</p>
        </div>

        <div className="bg-white dark:bg-[#1A1414] p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm space-y-2">
          <Trophy className="w-6 h-6 text-[#2E8B57]" />
          <h3 className="font-bold text-sm text-gray-900 dark:text-white">{language === 'hi' ? 'पुरस्कार व पदक' : 'Medals & Prizes'}</h3>
